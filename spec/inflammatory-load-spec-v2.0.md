# Dietary Inflammatory Load — Normative Spec v2.0

**Supersedes:** v1.9. A refused record's declared values now prefill the manual form (§8.5):
in the common refusal — bread whose flour mix the label does not state — the label is already
in the record, and the user supplies only what the record could not determine. §13.5 gains the
copy for that case. §8.6 states the migration-stamp rule that until now lived only in a test.

Previously (v1.9): answers to the v1.8 report. §6.1b renders the quantity as entered —
v1.8 stored it and showed it nowhere. §3.3a states that step 1 is the only density route for a
USDA record and that steps are tried 1, 2, 2b. §3.3's unit table becomes exact. §8.2 stops
claiming the app is fully functional without a USDA key, because finding a generic whole food
is not. §6.6's verdict-word list becomes one list, applied to rendered text and never to
identifiers.

Previously (v1.8): the first findings from real use rather than review, all three from
trying to log a morning coffee: §8.5b adds combos, §3.3 and §3.3a add US customary volume units
and `BDMAP-1` for scoopable solids, and §8.1 fixes search result order and labelling. §3.3 named
gram entry as the friction most likely to stop logging; it was the first thing that did.

Previously (v1.7): entry removal enters the spec: §8.4 permits removing an entry dated
today and refuses every prior day, §6.7 adds the removal hint, and AV-25 covers it. §2.5 gains
its fifth form — a check that has never judged an instance has never been tested — after §4.4's
palette guard was found to have rejected every colour declaration, including the ones it named
as permitted, from v1.0 to v1.6.

Previously (v1.6): the first defect found by use: Open Food Facts text search was routed to
an endpoint that silently ignores the query. §8.1 now states which endpoint text search uses and
why, and §2.5 records that a fixture cannot see a service's behaviour.

Previously (v1.5): closing edits only: §2.5 gains the surface-coverage rule, `[OPEN-6]`
...
**Fixtures cannot see a service's behaviour.** A test against a fixture verifies that the code
handles a response correctly; it cannot verify that the service returns that response for that
request. Where correctness depends on a live service honouring a parameter, one test must
exercise the live service and assert the parameter took effect. The v2 search defect passed
every fixture test while returning mineral water for a cereal query.
...
**A check that has never judged an instance has never been tested.** A structural check can
engage correctly, match, and still be wrong, if the codebase has never presented it with a real
case. §4.4's palette guard rejected every colour declaration — including the values it named as
permitted — from v1.0 to v1.6, because no `.entry` rule declared a colour until v1.6. Nothing
could have revealed it except the first real case.

Every structural check therefore carries its own discrimination cases: at least one value it
must accept and one it must reject, asserted inline on every run. The check is exercised whether
or not the codebase currently supplies an instance, and a check that separates nothing fails the
build.

This is B1's discrimination rule applied to structural checks, and it is the fifth form of one
lesson: a check must be able to fail on its defect (B1); match tokens, not substrings (Y4);
prove its mechanism engaged (H3); state the surface it covers (v1.5); and prove it judges
correctly, not merely that it runs.
...
**Volume units.** Quantity may also be entered in US customary volume units, converted exactly
to millilitres before §3.3a runs:

| Unit | Definition | Millilitres |
|------|------------|-------------|
| fl oz | US customary | 29.5735295625 |
| cup | 8 fl oz | 236.5882365 |
| tbsp | ½ fl oz | 14.78676478125 |

These are exact. The same constant is used on every path — user-entered quantities and
source-declared serving sizes alike — so a given volume converts to one mass regardless of how
it arrived.

Fractional values are accepted in all units (`0.5`, `1/3`, `1 1/2`). Teaspoons are not offered:
⅓ tbsp is one teaspoon.

The entry stores the unit and quantity as entered and displays them that way (`12 fl oz`,
`1/3 tbsp`). `quantity_g` is derived and stored as always.
...
1. **Per-product derivation.** If the source record supplies both a serving mass in grams and
   ...
   For USDA records, step 1 is the **only** density route: USDA records carry no
   `categories_tags`, so steps 2 and 2b can never match them. The mass and volume come from
   `foodPortions`, returned only by the detail endpoint `/food/{fdcId}`, not by search; a
   selected USDA result is re-read from the detail endpoint before scoring.

   In SR Legacy portions, `measureUnit.name` may be the literal string `"undetermined"` with
   the real unit in `modifier`. `"undetermined"` is not a unit and must not be read as one.
...
2b. **Scoopable solids.** A solid entered by volume converts to mass through
    `bulk-density-map.json` (`BDMAP-1`), a versioned static file keyed on the product's declared
    `categories_tags` (§7.2a), holding a bulk density in g/ml per class. Initial classes:
    granulated sugar, brown sugar, flour, rolled oats, rice (dry), nut butter, honey, ground
    coffee, cocoa powder.

    **Bulk density is less precise than liquid density.** A tablespoon of oats varies with how
    it is packed; a tablespoon of water does not. The entry stores `density_class` and
    provenance `BDMAP-1`, so a spooned quantity is distinguishable from a weighed one in any
    audit. The method page says so.

    A solid whose class does not resolve **cannot be entered by volume**: the volume units are
    disabled for that product and the form offers grams only. This is a form constraint, as K2's
    alcohol rule is (§8.5) — the app never guesses a density to make a unit available.

    **Order.** Steps are tried in order: 1, 2, 2b. A product with a liquid class converts at
    its liquid density; `BDMAP-1` is consulted only when no liquid class resolves. A product
    resolving under none of them is not enterable by volume.

    Step 2b is keyed on `categories_tags` and therefore applies to Open Food Facts products
    only. For USDA products, step 1 is the sole route to a density, for solids entered by
    volume as for liquids.
...
The entry stores `density_used` and its provenance (`DERIVED` | `DMAP-1` | `BDMAP-1` | `MANUAL`).
...
### 6.1b Entry quantity line

Rendered beneath §6.1's first line, above drivers, on every entry:

```
{quantity_as_entered}
```

The quantity and unit exactly as the user entered them: `12 fl oz`, `1/3 tbsp`, `45 g`,
`1 package`. Fractions are reconstructed for display only from the culinary denominators 2, 3,
4 and 8, on an exact match; otherwise the decimal is shown to at most two places with trailing
zeros stripped.

The derived `quantity_g` is **not** shown. The user entered a volume; showing a mass they did
not enter invites the question of where it came from, and the method page answers that for
anyone who asks.

For combo component entries, the combo name precedes the group (§8.5b); each component still
shows its own quantity line.
...
**One list, applied to rendered text.** §6.6's verdict-word list lives in one module and is
imported by every check that enforces it. Checks apply it to rendered, user-visible text — never
to ids, class names, or identifiers, which the user never sees. A verdict word in an element id
is not a verdict.

Two partly-right lists is §9's duplication problem in test form, and the false positive that
exposed it — the element id `add-choose` matching `choose` — is the same mistake in the other
direction: a prohibition applied to a surface the prohibition was never about (§2.5).

### 6.7 Removal hint

Rendered on the Today view whenever it lists at least one entry:

```
Press and hold an entry to remove it. Today only — earlier days are final.
```

The affordance is uniform across today's entries (§7.1's argument applies): it says nothing
about any entry that it does not say about every entry.
...
**What "uniform" is measured over.** §6.7's removal affordance is present on today's entries
and absent on every prior day, and this argument still holds, because uniformity is required
across the set an affordance applies to, not across the whole store. A control keyed to the
calendar date says nothing about the food; a control keyed to score, band, or attribute does.
The test is whether the affordance's presence could be read as a claim about the entry carrying
it — not whether every entry in the store carries it.
...
**Combo-logged entries count normally.** A product logged daily through a combo (§8.5b)
contributes to its history exactly as direct logs do. This does not let it dominate the corpus:
the corpus is distinct products, so a product logged thirty times is one candidate. Frequency
affects only whether a product clears the two-entry threshold and what its median
`REFERENCE_MASS` is — and a median set by the combo quantity is the user's real portion, which
is what `REFERENCE_MASS` exists to capture.
...
**Text search and barcode lookup use different endpoints.** Barcode lookup uses the product
endpoint and is unaffected by what follows.

Text search must use an endpoint that both honours a free-text query **and returns the full
product record**. The v2 search endpoint satisfies neither: it supports structured filters only
and silently ignores a text query, returning an unfiltered global result set. A text search
routed to v2 is a defect even though it returns results.

Text search uses the **v1 search endpoint** (`/cgi/search.pl`). It honours the query and returns
`nutriments`, `nutrition_data_per`, `quantity`, `nova_group` and `ingredients` — every field
§3.3c, §3.1 and §2.1 need.

Search-a-licious (`search.openfoodfacts.org`) honours the query but returns a search index
document, not a product record: no nutriments, no basis, no NOVA group. Using it would require a
second lookup per result and still yield sparse records — a measured example returned zero
nutriments for the first hit — so it is not used. It is documented here so the next reader does
not reach for it on the assumption that newer is better.

**Text search is filtered to United States products** (`countries_tags_en=United States`). Open
Food Facts is global, and without the filter a working search still surfaces regional variants
the user cannot buy. The filter applies to text search only: a scanned barcode resolves
regardless of country, because the product is physically in the user's hand.

**Result order and labelling.** When a USDA key is configured (§8.2), USDA Foundation Foods and
SR Legacy results are listed before Open Food Facts results. Within each group, the source's own
order is kept. The order is fixed, not inferred from the query.

Every result shows its source (`USDA` or `Open Food Facts`), its brand where one exists, and its
energy per 100 g or 100 ml — enough to tell a plain brewed coffee from a flavoured bottled drink
at a glance.

For packaged products, barcode scanning is the preferred route and bypasses search entirely. The
search screen says so where a scan is available.
...
Without a key, the app scores and records correctly, but text search for **generic whole
foods** (brewed coffee, granulated sugar, an apple) returns branded Open Food Facts products
instead, because Open Food Facts is a catalogue of packaged goods. Barcode scanning is
unaffected. The key is optional for correctness and close to necessary for convenience.

With no key configured, resolution uses `OFF` alone and unresolved products fall to `MANUAL`
(§8.5) as they otherwise would; `USDA` search returns `NO_API_KEY` and is skipped.
...
**Removal.** An entry dated today may be removed. An entry on any prior day may not: removal is
the stronger operation than mutation, and "prior days are read-only without exception" governs
both.

The line is principled, not merely consistent. Today's entries are exactly those whose figures
have never been judged: `TODAY_LOAD` is unbanded (§6.2a), today is excluded from `WINDOW_LOAD`
(§1.2) and from §4.6 normalization, and the current trend block is not plotted (§4.5). Removing
a today entry therefore changes only figures the app has marked as provisional, and never
rewrites a band, summary, normalized figure, or plotted block the user has already been shown.

A removal on a prior day is refused with `PRIOR_DAY_READ_ONLY`. A removal of an unknown entry is
refused with `NO_SUCH_ENTRY`, so a removal that removed nothing cannot report success.

Removal is not an edit path. No field of a stored entry becomes mutable, and a correction is
still a new entry.

A removed entry is erased, not tombstoned. The audit trail's obligation is that every figure the
app has shown traces to the entries that produced it; an entry removed on its own day
contributed to no judged figure. Erasure also keeps an accidental entry out of the swap corpus
(§7.2) and out of median `REFERENCE_MASS`.

Removing every entry on the day `TREND_EPOCH` was set leaves the epoch in place with no entries
on that date. This is correct: the epoch pins the grid (§4.5) and does not assert that data
exists on its date.

One consequence follows and is stated so it is not mistaken for drift: §4.5's
epoch-does-not-move clause is now reachable only on the day the first entry was logged, because
after that day the entry cannot be removed at all. What the clause guarantees is unchanged; the
window in which it can fire is narrower.
...
**Prefill from a refused record.** When a product refuses for a reason other than its basis —
`GRAIN_MAJORITY_UNKNOWN` (§3.1), an unresolved liquid or bulk density class (§3.3a), or an
ineligible dataset (§8.2) — and the record's basis resolved under §3.3c, the manual form is
prefilled with every nutrient and macro value the record declares, converted to the form's
per-serving basis. Classification attributes the record declares (e.g. `P5` from `nova_group`)
are prefilled too. The user supplies only what the record could not determine — for bread,
whether the grain is whole or refined — and may edit any prefilled value.

**No prefill when the basis itself refused** (§3.3c rule 4). Nutrient figures without a known
basis are numbers without units, and carrying them forward would present them as meaningful.

**A prefilled field is a value from the record, not a user claim, until the form is submitted.**
Fields the record did not declare arrive blank and follow the J4/J5 rules exactly — blank is an
unfinished form, never 0.

**Provenance.** The resulting entry is `source: MANUAL` and additionally stores
`prefilled_from` — the refused record's source and product id — and the list of fields the user
changed from their prefilled values. An entry built from a record is thereby distinguishable, in
any audit, from one typed off a label.

Saving a prefilled manual entry as a saved product (§8.5a) works as for any manual entry.
Rescanning the same barcode offers the saved product first.
...
A combo is a named, saved list of components, each a product with a quantity and unit. Logging a
combo writes one ordinary entry per component, in a single operation, all with the same
timestamp. It creates no composite record: §3.7 holds, and each component entry is scored,
stored, and audited exactly as if it had been logged alone.

**Components are snapshotted at combo creation**, as saved products are (§8.5a). Each component
stores the resolved record it was built from, so logging a combo needs no network, produces the
same entries every time, and is unaffected by later edits to the source record. Editing a combo
affects future logs only; entries already written are immutable (§8.4).

**Component entries keep their original source and `product_id`.** A coffee logged through a
combo and a coffee logged directly are the same product, so both count toward that product's
history for §7.2's corpus threshold and median `REFERENCE_MASS`. A combo is not a source.

Each component entry stores `combo_id` and the combo's name at log time, so the Today view can
group them (§3.7 permits grouping same-timestamp entries). Grouping is presentational only.

**Removal** follows §8.4: today's entries only. Removing one component removes that entry alone;
the view may also offer removing the whole group, which removes each of today's component
entries individually.

Logging a combo is atomic: if any component fails to write, none are written.
...
- **Storage: IndexedDB**, not localStorage. §8.4 requires immutable per-entry snapshots plus
  ...
  Schema is versioned with an explicit migration path. `SCHEMA-4 → SCHEMA-5` adds
  `prefilled_from` and `prefill_changed` (§8.5); like the combo fields, they are present only
  where they apply, and their absence reads as "not prefilled" and needs no backfill.
  `SCHEMA-3 → SCHEMA-4` adds `combo_id`
  and `combo_name` (§8.5b) and a combo store; entries written before it carry neither field,
  which reads as "not logged through a combo" and needs no backfill. `SCHEMA-1 → SCHEMA-2` adds
  ...
  **A migration hop stamps its own target version**, as a literal — never the
  current `SCHEMA_VERSION`. A hop that stamps the current version claims a shape it
  knows nothing about, and makes every later hop skip: the chain tests
  `schema_version < N` and a prematurely stamped entry fails that test for every
  remaining N. This has been introduced twice and is enforced mechanically.
...
### AV-25 — Removal is today-only

The store holds two entries dated today and one dated earlier. `TREND_EPOCH` is the earlier
date.

Removing a today entry removes that entry and no other. The day's load becomes the sum of the
surviving today entries. The other entries are unchanged field for field.

Removing the earlier entry is refused with `PRIOR_DAY_READ_ONLY`, and the entry is still stored
afterwards. Removing an id that is not in the store is refused with `NO_SUCH_ENTRY`.
`TREND_EPOCH` does not move.

**Discrimination.** The refusals are the discriminating assertions, and they are categorical
rather than numeric: with the prior-day guard removed, the prior-day entry is deleted and both
the refusal assertion and the still-stored assertion fail. A guard that deleted and then threw
would pass the refusal assertion alone, which is why the still-stored assertion is separate.

The removal assertions discriminate against a delete that removes too much: a delete emptying
the store satisfies "the entry is gone" and fails "only the named entry is gone".

Verified by mutation: with the guard removed, 2 of 8 assertions go red.

### AV-26 — A combo writes ordinary entries

A combo named "Morning coffee" holds three components: `Coffee, brewed` 12 fl oz (USDA),
`Sugar, granulated` 2/3 tbsp (USDA), `Lactaid milk` 2 fl oz (OFF). Logging it writes **three**
entries, not one, all on the same local date and carrying the same `combo_id` and `combo_name`.

Each entry's `source` is the component's original source — `USDA`, `USDA`, `OFF` — never a combo
source, and each `product_id` is the product's own. A coffee already logged directly therefore
shares a `product_id` with the combo's coffee: with one direct entry and one combo entry, that
product has **two** stored entries and enters §7.2's corpus.

**Discrimination.** The defect this vector exists to catch is a combo implemented as a composite:
one entry whose values are the sum of its parts. That defect writes **1** entry where the correct
implementation writes **3**, and leaves the coffee product with **1** stored entry where the
correct implementation leaves **2** — below §7.2's two-entry threshold, so the product does not
enter the corpus and no swap is ever offered for it. Both are categorical.

A second defect writes three entries but stamps them `source: SAVED` or a combo `product_id`. That
yields the right entry count and the wrong corpus: the coffee product has 1 entry under its own
id, so §7.2 still excludes it. The `product_id` assertion separates these two.

### AV-27 — A combo write is atomic

The same combo, with its third component made unwritable (a duplicate `entry_id`, which §8.4
refuses with `ENTRY_EXISTS`).

The store holds **zero** new entries afterwards. Not two.

**Discrimination.** The correct implementation leaves 0 entries; the non-atomic defect leaves 2.
Categorical, and the difference is the whole point: a partially written combo is a silently wrong
day total, which §8.4 cannot correct afterwards because entries are immutable and only today's
may be removed.

`TREND_EPOCH` is unchanged by a refused combo on an empty store: no entry was written, so none
was first.

### AV-28 — Volume entry in fluid ounces

`Coffee, brewed`, a water-based beverage (`DMAP-1` density 1.00), entered as **12 fl oz**.

12 fl oz converts to **354.882 ml**, which at density 1.00 gives `quantity_g` = **354.882**.

The entry stores `quantity_value` 12 and `quantity_unit` `fl oz` — as entered, not as converted —
and §6.1 renders `12 fl oz`. `quantity_g` is the single canonical quantity (§3.3a) and everything
scales from it.

**Discrimination.** The defect is treating a fluid ounce as an ounce of mass: 12 oz = 340.194 g,
a delta of **14.688 g** against the correct 354.882 g, which propagates to every nutrient and to
the score. A second defect stores the converted value as the entered one and renders
`354.882 ml`, which differs from `12 fl oz` as a string.

### AV-29 — Volume entry for a scoopable solid

`Sugar, granulated` entered as **1/3 tbsp**.

The record supplies no per-product derivation (step 1) and no liquid class (step 2), so §3.3a step 2b applies. 1/3 tbsp is 4.929 ml, and `BDMAP-1` gives
granulated sugar 0.845 g/ml, so `quantity_g` = **4.165**. The entry stores `density_used` 0.845
and `density_provenance` `BDMAP-1`, distinguishing a spooned quantity from a weighed one.

A product resolving under neither §3.3a step 2 nor step 2b **offers grams only**: the volume units
are absent from the form, and a volume quantity submitted for it is refused rather than converted
at an assumed density.

**Discrimination.** The defect is assuming a bulk density of 1.00: `quantity_g` = 4.929 against
the correct 4.165, a delta of **0.764 g** — 18% of the quantity, and the same 18% on every scaled
nutrient. The refusal case is categorical: entry refused versus entry created at a guessed
density.

### AV-30 — The quantity renders as entered

`Coffee, brewed` logged at **12 fl oz**, resolving through §3.3a step 1 to `quantity_g` 372.0
(USDA `foodPortions`: 1 cup = 248 g, so density 1.0482).

§6.1b renders `12 fl oz`. It does **not** render `372 g`, `372.0`, or `354.882 ml`.

§6.1's first line is unchanged by the addition: it is still `{food_name} — {score:+0.1f}` and
carries no quantity.

A second entry of `Sugar, granulated` at **1/3 tbsp** renders `1/3 tbsp` — the fraction
reconstructed, not `0.33 tbsp`.

**Discrimination.** Three defects, all categorical and all visible as strings:

- Rendering the derived mass shows `372 g` where `12 fl oz` is correct. These differ in both
  number and unit, so a string assertion separates them.
- Rendering the converted volume shows `354.882 ml`, correct as a volume and wrong as the
  quantity entered.
- Appending the quantity to §6.1's first line instead of its own line yields
  `Coffee, brewed — +0.4 · 12 fl oz`, which contains the right substring and the wrong §6.1
  format. The vector asserts §6.1's first line separately for exactly this reason.

A fourth, on the fraction: rendering `0.33 tbsp` where `1/3 tbsp` was entered is a rounding of
the entered value, not a rendering of it.

### AV-31 — A grain refusal arrives prefilled

An Open Food Facts bread record: `nutrition_data_per` `100g`, package 567 g, serving
`1 slice (38 g)`, ingredients listing whole wheat flour and enriched flour with no declared
percentages. It refuses with `GRAIN_MAJORITY_UNKNOWN` (§3.1); its basis resolves as `per_100g`.

Declared per 100 g: added sugar 7.9 g, sodium 447 mg, saturated fat 0.5 g, fibre 5.3 g, energy
263 kcal, protein 10.5 g, carbohydrate 47.4 g, fat 3.9 g.

The manual form arrives with serving mass **38 g** and every field converted to that serving:
added sugar 3.0 g, sodium **169.86 mg**, saturated fat 0.19 g, fibre 2.01 g, energy 99.94 kcal,
protein 3.99 g, carbohydrate 18.01 g, fat 1.48 g. The grain choice is the only unanswered
input. Choosing whole grain produces `A7`; choosing refined produces `P4`. Submitting with
neither chosen is refused — the choice is what the refusal was about.

**Discrimination.** Three defects:

- Carrying the per-100 g values into a per-serving form unconverted: sodium **447** where
  **169.86** is correct, a delta of **277.14 mg**, and the same factor of 2.63 on every field.
- No prefill at all: 0 fields filled against 8. Categorical.
- Accepting the form with no grain chosen: an entry created with neither `P4` nor `A7`, against
  a refusal. Categorical, and it silently discards the one fact the refusal existed to obtain.

### AV-32 — A basis refusal prefills nothing

A record with no `nutrition_data_per` refuses on §3.3c rule 4. The manual form arrives with the
product name only; every nutrient and macro field is blank.

A record that would refuse on **both** basis and grain majority is reported as a basis refusal.
The grain copy (§13.5) says "Everything else is filled in", which would be false over an empty
form.

**Discrimination.** Prefilling a basis refusal carries eight numbers without units into the
form: 8 fields against 0. Reporting the double refusal as a grain refusal renders a sentence that
is untrue on the screen it appears on. Both categorical.

### AV-33 — A partly declared record prefills only what it declares

A record refusing on `GRAIN_MAJORITY_UNKNOWN` with a resolved basis declares six of the eight
fields; `added_sugar_g` and `fiber_g` are absent. The form arrives with six fields filled and
those two blank.

Submitting without addressing the two blank fields is `FIELD_NOT_STATED`, and no entry is
written. Marking them "not stated" creates an `INCOMPLETE` entry listing both.

**Discrimination.** A blank field read as 0 creates an entry claiming zero added sugar and zero
fibre, marked `COMPLETE`, where the correct outcome is a refusal. Categorical — and it is the
worse of the two errors, since a false `COMPLETE` hides the gap §6.4 exists to show.

### AV-34 — A prefilled entry records its provenance

AV-31's bread, submitted with whole grain chosen and nothing else edited, stores
`source: MANUAL`, `prefilled_from` `{ source: OFF, product_id: off:<code> }` and
`prefill_changed` **empty**. The same form with sodium edited to 170 stores `prefill_changed`
`["sodium_mg"]` — exactly that field. The grain choice is not a change: it was never prefilled.

**Discrimination.** Two defects:

- Omitting `prefilled_from`: the entry is indistinguishable from one typed off a label.
  Categorical.
- Comparing the submitted value against the unrounded record value rather than the value the
  form displayed: every field whose conversion is not exact registers as changed. For this
  bread that is **4** of the 8 — added sugar (3.002 shown as 3), fibre (2.014 → 2.01),
  carbohydrate (18.012 → 18.01) and fat (1.482 → 1.48) — where 0 is correct, which makes the list
  useless as evidence of what the user did.
...
1. §3.3c basis resolution, §3.3a, §3.3b, §3.3, §3.1, §3.6. Vectors AV-1, AV-4, AV-6A, AV-6B,
   AV-6c, AV-7, AV-7b, AV-8, AV-9, AV-9a, AV-10, AV-14, AV-15, AV-28, AV-29.
   ...
   assigned elsewhere.
2. §8.4 / §8.6 IndexedDB storage, `SCHEMA-2`, immutability, §8.6a carve-out. Vectors AV-21,
   AV-25.
3. §8.1 / §8.2 source resolution; §8.5 manual entry, including prefill from a refused record;
   §8.5a saved products. Vectors AV-7a, AV-11, AV-12, AV-13, AV-31, AV-32, AV-33, AV-34.
4. §2.5 macro capture; §4.6 normalization. Vectors AV-16, AV-17, AV-18.
5. §4, §6 display and banding, including §6.1a, §6.1b, §6.2a, §6.2b, §6.3a, §6.3b. Vectors
   AV-2, AV-5, AV-8, AV-22, AV-30. Also §3.5 juice field selection and its §2.4 display-name consequence. Step 1 fixtures
...
7. **Logging flow (§13.2).** Barcode scan, text search, quantity entry, manual entry, saved
   products, and §8.5b combos. The first path by which a user creates an entry.
   Vectors AV-26, AV-27.
...
- **Settings.** The optional USDA API key (§8.2), and nothing else. This screen exists for that
  ...
  Where the key is entered, the screen states:

  ```
  Optional, free, and stored on this device only. Without it, searching for plain foods like
  coffee or sugar returns branded products instead. Scanning barcodes works either way.
  ```
...
**`GRAIN_MAJORITY_UNKNOWN`.** The copy says what is needed rather than what failed:
