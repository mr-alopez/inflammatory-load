# Dietary Inflammatory Load — Normative Spec v0.8

**Supersedes:** v0.7. Resolves S1–S6 and T1–T7. Promotes `AV-6A`/`AV-6B` to real headings
(S1), corrects §3.3c's rule 2 error magnitude and sign (S2), exempts `NOT_APPLICABLE` entries
from `INCOMPLETE` (S4), and makes snapshot immutability explicitly deep (§8.4).

## Changed vector IDs

`AV-6` is **retired**. Its two sub-entries become real headings, `AV-6A` and `AV-6B`, with
the content they already had. Nothing renumbers: §11, §10's precision block and every existing
fixture already cite `AV-6A` and `AV-6B`, so those IDs become valid rather than changing. No
fixture is pinned to bare `AV-6`.
