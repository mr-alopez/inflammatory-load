# Dietary Inflammatory Load — Normative Spec v0.6

**Supersedes:** v0.5. Resolves B1–B6 and C1–C6. Respecifies `AV-6B` so it can detect the
defect it names (B1), adds the §10 discrimination convention, resolves the `MANUAL`
volume-only basis hole (§3.3c), and corrects the §11 step 1 vector list.

## Changed vector IDs

None. `AV-9a` is inserted after `AV-9` and uses a suffix, so no existing vector ID moves.
Every fixture pinned to a v0.4 or v0.5 ID remains valid against v0.6.

`AV-6B` keeps its ID but its **expected values changed**: its beverage is respecified from a
water-based cola at density 1.00 to a juice-class drink at density 1.04, so that the
apply-density-once defect no longer coincides with the correct result (B1). Any fixture pinned
to `AV-6B` must be updated, not merely re-run.
...
`density_class` that selected it where provenance is `DMAP-1`; the full attribute value set as
retrieved **and** as consumed,
...
`P3`-bearing values are non-terminating and are printed exactly:
`P3` at 355 ml / 5.0% ABV = 2.0148785714285714…
`P3` at 44 ml / 40% ABV  = 1.9837714285714285…
A fixture may assert these at any tolerance down to machine epsilon.
...
**Entry B — orange juice drink, 240 ml.** Basis `per_100ml`, density 1.04 (`DMAP-1`, juice),
`quantity_g` = 249.6. `P5` serving mass: labeled serving 240 ml → 249.6 g → 1.0 serving.
...
serving derivation, and that density cancels (10.8 × 240 / 100 = 25.92, independent of the 1.04
density).
...
5. §4, §6 display and banding, including §6.1a, §6.2a, §6.2b, §6.3a, §6.3b. Vectors AV-2, AV-5,
   AV-8. Also §3.5 juice field selection and its §2.4 display-name consequence. Step 1 fixtures
