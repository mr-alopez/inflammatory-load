# Dietary Inflammatory Load — Normative Spec v1.0

**Supersedes:** v0.9. Resolves X1–X4 and Y1–Y5, **resolves `[OPEN-7]`** (swap candidate
...
If the corpus contains no eligible candidate in the source entry's category:
...
date; `macro_basis` (`SCHEMA_2` | `PRE_SCHEMA_2`, §8.6a); and `food_name` as resolved at
write (§6.0a).
...
No candidate is returned and no §6.5 line is rendered. An implementation falling back to a
curated list, an external database, or the §7.3 no-clear-alternative string is a defect — the
corpus is empty, not the comparison unfavourable.
