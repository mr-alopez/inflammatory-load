# Dietary Inflammatory Load — Normative Spec v0.5

**Supersedes:** v0.4. Resolves K1–K10. Fixes the macro summation basis (§6.2b), closes the
`P3`-without-volume input hole (§8.5), and adds source-basis provenance (§3.3c, §8.4).

## Changed vector IDs

None. `AV-6c` is inserted after `AV-6B` and uses a suffix, so no existing vector ID moves.
Every fixture pinned to a v0.4 ID remains valid against v0.5.
...
2. `nutrition_data_per == "100g"` **and** the product's `quantity` unit is `ml`, `l`, `cl`, or
   ...
   **Known failure mode.** A product sold by volume whose nutrition table is genuinely per
   100 g is divided by density by §3.3b, producing a silent error equal to (density − 1): up
   to 4% on juice, 3% on milk, 9% on an oil sold by volume. The error is not detectable from
   the record. This is why the provenance is stored (§8.4): a `DERIVED_RULE_2` entry is the
   only value in the pipeline that can be wrong without leaving a trace, and it must be
   identifiable in an audit or an export.
...
Totals include only non-null entry values, except as carved out in §8.6a.

If any entry that day is missing a macro field, the line appends:
...
unit; `quantity_g`; `density_used` and its
...
- `source_basis` and its provenance are null on these entries and are omitted from any audit
  or export view rather than rendered as unknown.
...
Supplying volume 44 ml and ABV 40% resolves the entry per AV-6A.
...
Same record as AV-12 with `fiber` = 5.0 g restored, and `protein_g` null.

| Field | Reported (per 100 g) | As consumed |
|-------|----------------------|-------------|
| `energy_kcal` | 250 | 250 |
| `protein_g` | `null` | — |
...
Scored attributes as AV-12 with `A1` = −1.0. Sum = −0.4. `SCORE` = **−0.4**
