# Dietary Inflammatory Load — Normative Spec v0.4

**Supersedes:** v0.3. Resolves M1–M11. Adds source-basis resolution (§3.3c), scopes the `P3`
pipeline exemption (§3.6), and adds a local saved-product store (§8.5a).
...
2. **`density-map.json`** (`DMAP-1`), a versioned static file keyed on the product's liquid
   classification (§3.3c). Values are physical constants for the class, not estimates of the
   individual product:
...
The basis is **read from declared fields, never inferred.** Resolution is ordered; the first
matching rule wins.
...
2. `nutrition_data_per == "100g"` **and** the product's `quantity` unit is `ml`, `l`, `cl`, or
   `fl oz` → `per_100ml`. OFF labels volumetric records `100g`; the product's own declared
   quantity unit is what distinguishes them, and it is a declared field, not an inference.
...
**`MANUAL`** (§8.5): always `per_serving`, with `serving_mass_g` equal to the mass the user
supplied.
...
All figures are integers. Totals sum only non-null entry values. If any entry that day is
missing a macro field, the line appends:
...
`alt_reference_mass_g` is the §7.2 `REFERENCE_MASS`, rendered as an integer. It is always
shown, so the displayed score is never attributable to a quantity the name implies but was not
used.
...
Every entry stores: resolved product ID; source (`OFF` | `USDA` | `MANUAL` | `SAVED`);
`source_basis`; quantity as entered and its unit; `quantity_g`; `density_used` and its
provenance where applicable; the full attribute value set as retrieved **and** as consumed,
including explicit nulls; all four macro fields including explicit nulls; `sugar_field_used`
where applicable; the resolved `OCCASION_CATEGORY` and the `CATMAP` version that produced it
(or `USER_OVERRIDE`); the coefficient set version; the computed score; and the local calendar
date.
...
A manual entry **can be `COMPLETE`.** If all supplied fields are non-null, no `INCOMPLETE`
marker applies.
...
- Logging from a saved product creates an entry with `source: SAVED` and copies the saved
  ...
  later does not alter any entry already logged from it.
...
assertion of absence; a vector testing missing data states `null` explicitly. Macro fields are
stated only in vectors that test them.

**Precision.** Contribution columns are shown display-rounded to **6 dp**; the exact value is
given where it differs. Fixture tolerance on contributions and sums is **1e-6**. Rendered
strings are compared for **exact equality**.
...
§6.2b renders `765 kcal` (5 × 152.65 = 763.25 → 763). Corrected: **763 kcal**.

Confirms full-precision summation before rounding, and that a high band triggers no message.

### AV-6 — Rum and coke, two entries (§3.7)

**Entry A — rum, 44 ml, 40% ABV.** Basis `per_100ml`, density 0.94 (`DMAP-1`, spirits),
`quantity_g` = 41.36. Ethanol = 44 × 0.40 × 0.789 = 13.886400 g → 0.991886 units.
...
**Entry B — cola, 240 ml.** Basis `per_100ml`, density 1.00 (`DMAP-1`, water-based beverage),
`quantity_g` = 240. `P5` serving mass: labeled serving 240 ml → 240 g → 1.0 serving.
...
Confirms that `P3` suppresses `P5` **only on its own entry**, that §3.3a conversion runs before
serving derivation, and that density cancels (10.8 × 240 / 100 = 25.92 regardless of the 1.00
density).
...
Basis `per_100g`, density 0.91 (`DMAP-1`, culinary oil), `quantity_g` = 13.65.
Note the basis is per **100 g**, so density does **not** cancel here — it applies once, on the
quantity side only.
...
(−0.328714), and the `A8`-only variant (−0.345714) all render `-0.3`, so a string assertion
passes on the defect this vector exists to catch.

### AV-11 — Missing data, pro-inflammatory

Entry resolves with `sodium = null`. `P2` contributes 0. Marked `INCOMPLETE`, renders
`Incomplete — missing sodium`. Score still computed, still counts toward `DAILY_LOAD`.

### AV-12 — Missing data, anti-inflammatory (symmetry)

Entry resolves with `fiber = null`. `A1` contributes 0. Marked `INCOMPLETE`, renders
`Incomplete — missing fiber`.

An implementation marking AV-11 but not AV-12 is a defect.

### AV-13 — Missing data, macro (symmetry)

Entry resolves with `protein_g = null`, all other fields present. Marked `INCOMPLETE`, renders
`Incomplete — missing protein` (§2.4 display name, not `protein_g`). The day's §6.2b line
appends ` · partial`.
...
- No `INCOMPLETE` marker on account of macro nulls; §6.4 lists nothing.
- The day's §6.2b line is **omitted entirely**, not rendered with ` · partial`.
- Any window containing this entry renders
  `Per 1,000 kcal: unavailable — some entries have no calorie data`.
...
1. §3.3c basis resolution, §3.3a, §3.3b, §3.3, §3.1, §3.6. Vectors AV-1, AV-4, AV-6A, AV-6B,
   AV-7, AV-7b, AV-9, AV-10, AV-14, AV-15.
...
- `[OPEN-7]` **Swap candidate corpus.** §7.2 ranks candidates but does not say where the set
  ...
  §8.5a saved-product store. §7 cannot be implemented until this is decided. Build with §7
  stubbed to the §7.3 suppression string.
