# Dietary Inflammatory Load — Normative Spec v0.7

**Supersedes:** v0.6. Resolves R1–R2, D1–D7 and E1–E7. Prints `P3` contributions and entry
sums separately (§10), makes entries explicitly append-only (§8.4), makes `PRE_SCHEMA_2` basis
fields absent rather than null (§8.6a), and gives §11's vector lists authority over any
handoff list (§11).

## Changed vector IDs

None. No vector is added, removed, or renumbered by v0.7's edits. Every fixture pinned to a
v0.4, v0.5 or v0.6 ID remains valid against v0.7.

Note that `AV-6B`'s expected values changed in **v0.6** — its beverage was respecified from a
